...
docker-start-service-debug docker-start-service-release: docker-start-service-%:
	$(DOCKER_COMPOSE) run -d --name backend --rm service_template-container make -- --in-docker-start-$*
	$(DOCKER_COMPOSE) run --rm -p 80:80 -p 443:443 --name nginx nginx-container

# Build and run service in docker environment
.PHONY: docker-stop-service-debug docker-stop-service-release
docker-stop-service-debug docker-stop-service-release: docker-stop-service-%:
	docker stop backend
	docker stop nginx
